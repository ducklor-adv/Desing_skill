...
### เปิดเครื่องมือ (Dashboard เปล่า ใช้กับแอปไหนก็ได้)
```bash
# เปิดกระดานออกแบบในเบราว์เซอร์ (ไฟล์เดียว ไม่ต้อง build)
start tool/design-board.html      # Windows  — เริ่มจากหน้าเปล่า
# open tool/design-board.html      # macOS
```
> เปิดมาเป็นหน้าเปล่า → กด **⬆ Import** เลือกไฟล์ `design.bundle.json` ที่ agent สร้างจากการสแกนแอป → ได้ทุกหน้าพร้อมรีดีไซน์
> (ตัวอย่างที่ map เสร็จแล้วของ TaskFlow อยู่ที่ `sample-app/design-board.html`)
...
├─ tool/
│   └─ design-board.html     ← ★ Dashboard เปล่า (generic) ใช้กับแอปไหนก็ได้ — Import bundle
